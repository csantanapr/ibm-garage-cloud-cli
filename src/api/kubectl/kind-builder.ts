...
export class DefaultKubeKindBuilder implements KubeKindBuilder {
  ...
  async getResourceNode<T extends KubeResource>(group: string | undefined, version: string, kind: string, namespace: string): Promise<KindClient<T> | undefined> {
    ...
    const versionNode = _.get(this.kubeClient, versionPath);
  ...
  async registerCrdSchema(name: string): Promise<boolean> {
    try {
      const crd: KubeBody<CustomResourceDefinition> = await this.kubeClient.apis['apiextensions.k8s.io'].v1beta1.customresourcedefinitions(name).get();

      if (!crd || !crd.body) {
        return false;
      }

      this.kubeClient.addCustomResourceDefinition({
        metadata: {
